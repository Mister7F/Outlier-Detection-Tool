...
class Tree:
    ...
    def impurity(self, debug=False):
        ...
        return (gini_1 * count_1.sum() + gini_2 * count_2.sum()) / self.dataset.shape[0]
    
    def split_data(self):        
        if self.feature_type[self.rule_col] == 'enum':
            self.group_1 = self.dataset[self.dataset[:, self.rule_col] == self.rule_val]
            self.group_2 = self.dataset[self.dataset[:, self.rule_col] != self.rule_val]
        else:
            self.group_1 = self.dataset[self.dataset[:, self.rule_col] >= self.rule_val]
            self.group_2 = self.dataset[self.dataset[:, self.rule_col] <  self.rule_val]
    ...
    def __str__(self):
        ...
        string = ('    ' if self.depth else '') + '[%s, %s %%] ' % (label, str(max(count)*100/count.sum())[:3])
