...
def perform_analysis():	
	for section_name in settings.config.sections():
		...
		logging.logger.info(str(len(y_data)) + ' - ' + str(len(y_data[0]))[:7] + ' - ' + str(len(y_data[0][0]))[:20])
		...
		# And add sum vector
		y_data = np.array([
			y[:model_settings["path_truncation"]] if y.shape[0] > model_settings["path_truncation"]
