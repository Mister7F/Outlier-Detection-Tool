...
def extract_model_settings(section_name):
    ...
    model_settings["layers"] = [int(k) for k in settings.config.get(
        section_name, "layers").split(',')]
    model_settings["epochs"] = settings.config.getint(section_name, "epochs")
    model_settings["fields"] = settings.config.get(
        section_name, "fields").split(',')
    model_settings["output_fields"] = settings.config.get(
        section_name, "output_fields").split(',')

    model_settings["ignore_history_window"] = settings.config.get(
        section_name, "ignore_history_window")
    model_settings["number_of_elements_displayed"] = settings.config.getint(
        section_name, "number_of_elements_displayed")

    # Test the model
    model_settings["test_the_model"] = settings.config.getint(
        section_name, "test_the_model")
    model_settings["test_label"] = settings.config.get(
        section_name, "test_label")
    model_settings["number_of_points_plotted"] = settings.config.getint(
        section_name, "number_of_points_plotted")
...
def perform_analysis():
    ...
    for section_name in settings.config.sections():
        ...
        for doc in es.scan(query_fields=model_settings["fields"] + model_settings["output_fields"]):
            ...
            _output_fields = es_to_numpy(
                doc['_source'], model_settings["output_fields"])
        ...
        logging.logger.info('Output fields shape  %i - %i' %
                            output_fields.shape)
        ...
        outliers.show_most_anormal(
            model_settings["number_of_elements_displayed"])
