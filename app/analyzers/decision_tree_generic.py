...
def extract_model_settings(section_name):
    ...
    model_settings["max_depth"] = settings.config.getint(
        section_name, "max_depth")
    model_settings["fields_type"] = settings.config.get(
        section_name, "fields_type").split(',')
...
def perform_analysis():
    for section_name in settings.config.sections():
        ...
        tree = Tree(
            dataset,
            model_settings["fields_type"],
            model_settings["max_depth"]
        )

        logging.logger.info('This tree want to classify %s' %
